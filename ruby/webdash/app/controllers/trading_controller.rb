...
class TradingController < ApplicationController
  ...
  def index
    ...
    @daily_notional = Price.connection.select_all("select coalesce(sum(abs(quantity)*price),0) from trade natural join turn where close_ts > now()::date;")[0]["coalesce"]
    @daily_notional = @daily_notional.to_i / 2
    @daily_closed_volume = Price.connection.select_all("select coalesce(sum(abs(quantity)),0) from turn natural join trade where close_ts > now()::date;")[0]["coalesce"]
    @daily_volume = @daily_closed_volume.to_i / 2
    if @daily_closed_volume.to_i != 0
      @daily_avg_profit_per_item = @daily_closed.to_i / (@daily_closed_volume.to_f/2)
      @daily_avg_profit_per_item = @daily_avg_profit_per_item.round(2)
    else
