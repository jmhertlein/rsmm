...
class TradingController < ApplicationController
  ...
  def index
    @total_profit = Price.connection.select_all("SELECT SUM(price * (quantity*-1)) AS total_closed FROM Trade NATURAL JOIN Turn WHERE close_ts IS NOT NULL and rs_type='osrs';")[0]["total_closed"]
    @total_volume = Price.connection.select_all("select sum(abs(quantity)) from trade where rs_type='osrs'")[0]["sum"]
    @total_notional = Price.connection.select_all("select coalesce(sum(abs(quantity)*price)/2,0) from trade where rs_type='osrs'")[0]["coalesce"]
    
    @daily_stats = Price.connection.select_all("select * from daily_history where rs_type='osrs' order by day desc limit 5")

    raw_long_term_stats = Price.connection.select_all("select day,closed_profit,volume from daily_history  where rs_type='osrs' order by day")
